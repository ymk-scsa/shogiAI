...
# 駒の利きを表現する関数
def make_attack_features(board: cshogi.Board, features: np.ndarray) -> None:
            ...
            for is_own in [True, False]:
                if (is_own and piece_color == board.turn) or (not is_own and piece_color != board.turn):
                    ...
                    for direction in piece_direction_list[1]:
                        ...
                        while True:
                            x = current_square // 9 + move_x
                            y = current_square % 9 + move_y * (-1 if not is_own else 1)
                            if not (0 <= y and y < 9 and 0 <= x and x < 9):
                                break
                            current_square = y + x * 9
                            features[BASE_INDEX + direction_offset + direction][x][y] = 1
                            if attack_board.piece_type(current_square) != 0:
                                break
